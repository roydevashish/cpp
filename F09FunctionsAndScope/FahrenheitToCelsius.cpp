...
and you need to just print the integer value . Just write the code that prints Fahrenheit to Celsius 
...
Start value is 120, end value is 200 and step size is 40. Therefore, the values we need to convert 
are 120, 120 + 40 = 160, and 160 + 40 = 200. 

The formula for converting Fahrenheit to Celsius is:
Celsius Value = (5/9)*(Fahrenheit Value - 32)  

Plugging 120 into the formula, the celsius value will be (5 / 9)*(120 - 32) => (5 / 9) * 88 
=> (5 * 88) / 9 => 440 / 9 => 48.88
...
float FahrenheitToCelsius(int f) {
    return (float)(f - 32) * ((float)5/(float)9);
}
...
int main() {
    ...
    for(int i = s; i <= e; i+=w) {
        cout << i  << "\t" << (int) FahrenheitToCelsius(i) << endl;
    }
